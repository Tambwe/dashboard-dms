    ...
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            <div class="flex items-center mb-4">
                ...
                <div>
                    <h2 class="text-3xl font-bold text-gray-900 dark:text-white">Modifier l'Organisation</h2>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                        Modifier les informations de {{ $organisation->name }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg overflow-hidden">
            <form action="{{ route('admin.organisations.update', $organisation) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="description" 
                              id="description" 
                              rows="3"
                              class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:text-white @error('description') border-red-500 @enderror">{{ old('description', $organisation->description) }}</textarea>
                    @error('description')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    @else
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            @foreach($clusters as $cluster)
                                @php
                                    $checked = in_array($cluster->id, old('cluster_ids', $selectedClusterIds));
                                @endphp
